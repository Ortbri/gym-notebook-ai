...
const WeekView = memo(({ item }: { item: Date[] }) => {
  const { handleInternalDateChange, setPage, days, activeDate } = useCalendar();
  ...
  const renderDay = useCallback(
    (day: Date) => {
      ...
      return <CalendarDay key={day.toISOString()} onPress={handlePress} day={day} />;
    },
    [handleInternalDateChange, setPage, days]
  );
  return <View style={styles.weekContainer}>{item.map((day) => renderDay(day))}</View>;
});
...
export const CalendarStrip = ({
  ...
  const { days, weeks, activeDate, handleInternalDateChange, stripRef, setPage } = useCalendar();

  const initialScrollIndex = useMemo(() => {
    const index = weeks.findIndex((week) => week.some((day) => isSameDay(day, activeDate)));
  ...
  const onViewableItemsChanged = useCallback(
    async ({ viewableItems }: { viewableItems: ViewToken<Date[]>[] }) => {
      ...
      const direction = currentIndex > previousIndexRef.value ? 'forward' : 'backward';

      let newDate: Date | null = null;

      const indexOfCurrentActiveDate =
        previousAvailableDates?.findIndex((day) => isSameDay(day, activeDate)) ?? 0;
      ...
      if (direction === 'forward') {
        ...
        const wasPreviousWeekLastDaySelected = isSameDay(previousActiveWeekLastDay, activeDate);

        // Ensure we have a valid date by providing a fallback
        const targetIndex = wasPreviousWeekLastDaySelected ? 0 : indexOfCurrentActiveDate;
        ...
        const wasPreviousWeekFirstDaySelected = isSameDay(previousActiveWeekFirstDay, activeDate);

        const targetIndex = wasPreviousWeekFirstDaySelected ? 6 : indexOfCurrentActiveDate;
