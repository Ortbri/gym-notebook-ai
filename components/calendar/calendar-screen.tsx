import { endOfDay, isWithinInterval, startOfDay, subDays, addDays, isSameDay } from 'date-fns';
...
const LazyLoadView = memo(
  ({
    ...
    item: Date;
    renderDay: ({ day, isActive }: { day: Date; isActive: boolean }) => React.ReactNode;
    style?: StyleProp<ViewStyle>;
  }) => {
    const { activeDate, offsetPageLimit } = useCalendar();
    const isActive = useMemo(() => isSameDay(item, activeDate), [item, activeDate]);
...
export const CalendarScreen = ({
  ...
  const { activeDate, days, handleInternalDateChange, pagerRef, stripRef, weeks } = useCalendar();
  ...
  const pageScrollHandler = useCallback(
    (e: {
      ...
      if (day && !isSameDay(day, activeDate)) {
        const originalIndex = weeks.findIndex((week) => week.some((d) => isSameDay(d, activeDate)));
        const newIndex = weeks.findIndex((week) => week.some((d) => isSameDay(d, day)));
  ...
  return (
    <PagerView
      ...
      {days.map((day) => (
        <LazyLoadView key={day.toString()} item={day} renderDay={renderDay} style={style} />
      ))}
    </PagerView>
  );
};
